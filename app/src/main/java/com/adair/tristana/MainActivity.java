...
public class MainActivity extends SwipeBackActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final OxTitlebar titleBar = (OxTitlebar) findViewById(R.id.titlebar);
        titleBar.setListener(new OxTitlebar.OnTitleBarListener() {
            @Override
            public void onClicked(View v, int action, String extra) {
                if (action == OxTitlebar.ACTION_LEFT_BUTTON || action == OxTitlebar.ACTION_LEFT_TEXT) {
                    onBackPressed();
                }
            }
        });

        titleBar.setDoubleClickListener(new OxTitlebar.OnTitleBarDoubleClickListener() {
            @Override
        ...
        if (position != 8) {
            maxAlphaEffectHeight = ScreenUtil.getScreenPixelSize(this)[1] / 3;
    ...
    private void initSmartTab() {
        OxTitlebar titleBar = (OxTitlebar) findViewById(R.id.titlebar);
